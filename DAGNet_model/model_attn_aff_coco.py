...
from .GSN import GroupedSpatialNormalization as GSN
...
class DAGNet(nn.Module):
    def __init__(self, num_classes=None, clip_model=None, embedding_dim=256, in_channels=512, dataset_root_path=None, device='cuda'):
        ...
        self.convgsn = GSN(3)
        self.daaf = DAAF(dim=self.embedding_dim)
    ...
    def forward(self, img, img_names, mode='train'):
        ...
        attn_fts = fts.clone()
        fts, attn_fts = self.daaf(fts, attn_fts)
        ...
        for i, img_name in enumerate(img_names):
            ...
            img_path = os.path.join(self.root_path, mode , str(img_name) + '.png')
            img_i = img[i]
