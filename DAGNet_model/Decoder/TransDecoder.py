...
#定义一个名为 DecoderTransformer 的类，继承自 PyTorch 的 nn.Module。
#该模块设计用于Transformer 解码器结构，多用于图像语义分割中的高层特征理解与输出预测。
class DecoderTransformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, output_dim: int):
        ...
        #一个 1×1 卷积层，将 Transformer 输出的特征图从 width 通道压缩或映射为 output_dim。
